...
/* ---------- 9판 꽃 마크(SVG) ---------- */
export function Bloom({ size = 96, filled = 9 }: { size?: number; filled?: number }) {
  const petals = [];
  for (let i = 0; i < 9; i++) {
...
/* ---------- 브랜드 헤더 ---------- */
export function Brand({ withAdmin }: { withAdmin?: boolean }) {
  return (
...
export function EnneagramApp() {
  ...
  const submit = useSubmitEnneagram({
    onSuccess: (r) => {
      router.push(`/enneagram/result/${r.id}`);
    },
  });
