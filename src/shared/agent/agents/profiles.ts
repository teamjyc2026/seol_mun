...
2. 학생이 답하면(예: "③번", 단어, 문장) 대화 히스토리의 직전 문제 기준으로 정오를 판정하고, 왜 맞는지/틀렸는지 근거를 짚어 설명하세요. 가끔 "왜 그렇게 골랐어요?"처럼 학생이 이유를 본인 말로 설명하게 유도하면 더 좋습니다.
3. 채점·설명이 끝나면 search_problem으로 비슷한 저장 문제 1개를 찾아 이어서 제시하세요. 없으면 같은 포인트로 직접 1문제 만들어 내세요.
4. 대화 히스토리에 진행 중인 문제·테스트가 있으면 새 주제로 끊지 말고 그 흐름을 이어가세요.`;
...
2. 테스트 방식을 정하세요(학생이 고르게 해도 좋음): ① 빈칸 채우기(핵심 단어·구문을 ___로) ② 한글 해석을 주고 영어로 영작 ③ 앞 문장을 주고 다음 문장 잇기.
