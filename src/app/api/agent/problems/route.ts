...
const subProblemSchema = z.object({
  ...
  core_content: z.string().max(4000).nullable().optional(),
  choice_explanation: z.string().max(8000).nullable().optional(),
...
const createSchema = z.object({
  ...
  passage_translation: z.string().max(20000).nullable().optional(),
  topic: z.string().max(100).nullable().optional(),
  difficulty: z.enum(DIFFICULTIES).nullable().optional(),
  problem_type: z.enum(PROBLEM_TYPES).nullable().optional(),
  question: z.string().min(1).max(4000),
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      passage_translation: body.passage_translation ?? null,
      question: body.question,
      choices: body.choices ?? null,
      answer: body.answer,
      explanation: body.explanation ?? null,
      core_content: body.core_content ?? null,
      choice_explanation: body.choice_explanation ?? null,
...
export async function PUT(req: NextRequest) {
  ...
  const rows = body.problems.map((p) => ({
    ...
    core_content: p.core_content ?? null,
    choice_explanation: p.choice_explanation ?? null,
    figures: p.figures ?? [],
    notes: p.notes ?? null,
